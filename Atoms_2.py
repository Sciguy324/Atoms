...
class Unit:
    """Utility class for utilizing units and unit conversions"""
    # Standard units:
    # Pressure: 1 atm
    # Mass: 1 gram
    # Volume: 1 L
    ...
    conversion_table = {"pressure": {"atm": 1,
                                     "kPa": 101.3,
                                     "in_Hg": 29.92,
                                     "mm_Hg": 760,
                                     "torr": 760,
    ...
    def __init__(self, symbol, category, standard=False):
        self.symbol = symbol
        self.category = category
        if category not in Unit.conversion_table:
            raise Warning("Warning: unit category is not present in conversion table")
        Unit.instances[category].append(self)

    def __repr__(self):
        return self.symbol

    def __mul__(self, x):
        """Initialize UnitFloats by multiplying regular numbers by a unit"""
        # Number is a scalar
        if type(x) in [int, float]:
            result = UnitFloat(float(x))
            result.units = {self: 1}
            return result

        # Number is a number with units
        elif type(x) == UnitFloat:
            result = x
            if self in result.units:
                result.units[self] += 1
            else:
                result.units[self] = 1
            result.fix_zeroes()
    ...
    def __div__(self, x):
        """Why on earth are you dividing a unit by a number?"""
        raise ArithmeticError("-This doesn't make any sense.\n-Not everything does, not everything has to")

    def __rtruediv__(self, x):
        """Divide by a unit to place it in the unit's denominator"""
        # Number is a scalar
        if type(x) in [int, float]:
            result = UnitFloat(float(x))
            result.units = {self: -1}
    ...
    def __rmul__(self, x):
        """Reverse-order multiplication support"""
        return self.__mul__(x)

    @classmethod
    def factor(cls, unit_a, unit_b):
        """Return a unitless conversion factor between the two units"""
        return cls.conversion_table[unit_a.category][str(unit_b)] / cls.conversion_table[unit_a.category][str(unit_a)]
        

    @classmethod
    def list_units(cls):
        for i, j in cls.instances:
            print("{}:".format(i))
            for k in j:
                print("\t-{}".format(k))
...
class UnitFloat(float):
    """Subclass of floats that have units"""
    ...
    def __repr__(self):
        ...
        for i, j in self.units.items():
            result += str(i)
            if j != 1:
                result += str(j).translate(sup)
        return result

    def __mul__(self, x):
        unitless_result = super().__mul__(x)
        # Other number is a unitless scalar
        if type(x) in [int, float]:
            result = UnitFloat(unitless_result)
            result.units = self.units.copy()
            return result

        # Other number is a number with units
        if type(x) == UnitFloat:
            result = UnitFloat(unitless_result)
            result.units = dict(Counter(self.units.copy()) + Counter(x.units.copy()))
            result.fix_zeroes()
            return result

    def fix_zeroes(self):
        """Removes units with a power of zero from the UnitFloat"""
        for i, j in self.units.copy().items():
            if j == 0:
                self.units.pop(i)

    def convert(self, unit_a, unit_b):
        """Converts from one unit to another"""
        factor = Unit.factor(unit_a, unit_b)
        print(factor)
        return factor * self / unit_a * unit_b
